// --- editor.js (Versión Final v2 - Autenticación Paciente) ---
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // --- PUNTO DE ENTRADA Y AUTENTICACIÓN (ESTILO main.js) ---
    // Este código se ejecuta SIEMPRE.
    console.log("Editor.js cargado. Esperando estado de autenticación de Supabase...");
    
    supabase.auth.onAuthStateChange(async (event, session) => {
        // Este listener se ejecutará cuando Supabase tenga claro si hay sesión o no.
        console.log(`Evento de Auth recibido: ${event}`);

        // Nos interesa el primer evento que nos confirma la sesión inicial
        if (event === 'INITIAL_SESSION') {
            if (session) {
                console.log("Sesión inicial encontrada. Verificando rol...");
                ...
                if (profile && profile.role === 'admin') {
                    // ¡ÉXITO! Es un admin. Ahora podemos iniciar el editor.
                    iniciarEditor();
                } else {
            ...
            // Si el usuario cierra sesión en otra pestaña, también lo redirigimos.
